...
# L1 balloon/glider ride — the only scenario that flies.
#
...
# `scenarios/l1_balloon_flight.jsonl`, byte for byte.  It was ALREADY
...
#
# Rides the screen-9 balloon: `glider_active` is assigned in exactly two
# places in the tree and this is the one that exercises the L1 path.  Visits
# ALL THIRTEEN L1 surface screens in 1,030 frames.
#
# LABELLED `slow` — 67s.  The engine paces to 18.2 Hz even headless, so frame
# count IS wall-clock: 1030 frames cannot run faster than it does.  The ten
...
#       --replay tests/fixtures/l1_balloon_flight.jsonl \
#       --trace tests/fixtures/golden_l1_balloon_flight.jsonl \
...
exec sh "$(dirname "$0")/lib/trace_gate.sh" golden_trace_l1_balloon_flight 1 - \
    l1_balloon_flight.jsonl golden_l1_balloon_flight.jsonl 1030 201 \
