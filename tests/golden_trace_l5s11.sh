...
# Display level 5, screen 11 — a CLAMP scenario for check_l3_transition.
#
...
# tool learned the lNsM_ filename convention, so `l5s11_walk_jump.jsonl`
# now runs `--level 5 --start-screen 11` on BOTH engines.
#
...
#   ./build/release/olduvai --play --level 5 --start-screen 11 \\
#       --replay tests/fixtures/walk_jump_in.jsonl \\
#       --trace tests/fixtures/golden_trace_l5s11.jsonl \\
#       --play-frames 300 --game-dir <game_dir>
#
# Skip (77) when game data or the binary is absent.

exec sh "$(dirname "$0")/lib/trace_gate.sh" golden_trace_l5s11 5 11 \
    walk_jump_in.jsonl golden_trace_l5s11.jsonl 300 180 \
