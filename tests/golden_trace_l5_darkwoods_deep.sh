...
# Display-L5 deep route — 30 attack presses.
#
...
# `scenarios/l5_darkwoods_deep.jsonl`, byte for byte.  It was ALREADY
...
#
# Display 5 = internal L3.  Ends in a game over.
#
# LABELLED `slow` — 24s.  The engine paces to 18.2 Hz even headless, so frame
# count IS wall-clock: 357 frames cannot run faster than it does.  The ten
...
#   ./build/release/olduvai --play --level 5 \
#       --replay tests/fixtures/l5_darkwoods_deep.jsonl \
#       --trace tests/fixtures/golden_l5_darkwoods_deep.jsonl \
...
exec sh "$(dirname "$0")/lib/trace_gate.sh" golden_trace_l5_darkwoods_deep 5 - \
    l5_darkwoods_deep.jsonl golden_l5_darkwoods_deep.jsonl 357 120 \
