...
# L2 arena WITH INPUT — 62 attack presses.
#
...
# `scenarios/l2_boss_fight.jsonl`, byte for byte.  It was ALREADY
...
#
# boss_golden_trace runs the same arena for 300 frames pressing NOTHING.
# This one actually fights: it is worth +30.5 points of boss_l2.cpp line
# coverage over the corpus without it, the single largest per-file gain of
# the ten adopted here.
#
# LABELLED `slow` — 28s.  The engine paces to 18.2 Hz even headless, so frame
# count IS wall-clock: 446 frames cannot run faster than it does.  The ten
...
#   ./build/release/olduvai --play --level 2 \
#       --replay tests/fixtures/l2_boss_fight.jsonl \
#       --trace tests/fixtures/golden_l2_boss_fight.jsonl \
...
exec sh "$(dirname "$0")/lib/trace_gate.sh" golden_trace_l2_boss_fight 2 - \
    l2_boss_fight.jsonl golden_l2_boss_fight.jsonl 446 120 \
