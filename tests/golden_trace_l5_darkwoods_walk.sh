...
# Display-L5 surface walk with attacks.
#
...
# `scenarios/l5_darkwoods_walk.jsonl`, byte for byte.  It was ALREADY
...
#
# LABELLED `slow` — 17s.  The engine paces to 18.2 Hz even headless, so frame
# count IS wall-clock: 230 frames cannot run faster than it does.  The ten
...
#       --replay tests/fixtures/l5_darkwoods_walk.jsonl \
#       --trace tests/fixtures/golden_l5_darkwoods_walk.jsonl \
...
exec sh "$(dirname "$0")/lib/trace_gate.sh" golden_trace_l5_darkwoods_walk 5 - \
    l5_darkwoods_walk.jsonl golden_l5_darkwoods_walk.jsonl 230 120 \
