...
| libmt32emu (munt) | [munt/munt](https://github.com/munt/munt) | LGPL-2.1 | **Linux AppImage only:** built from munt source (tag `libmt32emu_2_8_3` — the corresponding source) and bundled as a *replaceable shared object* (`libmt32emu.so.2` — swap it to exercise LGPL §6 relinking). MT-32 output additionally needs the user's own Roland ROMs. Elsewhere loaded at runtime (`dlopen`) from the user's system when present; never statically linked. |
