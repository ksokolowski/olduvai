...
#include "presentation/menu_render.hpp"      // draw_menu_vector/confirm_vector
#include "presentation/pause_service.hpp"    // PauseService
#include "presentation/screens.hpp"          // enhance::HdText
#include "presentation/text_overlay.hpp"     // TextOverlay
#include "presentation/widescreen_presenter.hpp"  // WidescreenPresenter, HudLayout
...
void FramePresenter::present(FrameBuffer& f, bool with_hud, bool do_present) {
    ...
    if (wide_frame) {
        ...
        // Classic (320×200): draw the cheat picker with the bitmap font into
    ...
    // composite).  Gate the picker on hd_text.ok() so it shows in HD even when
    // the hd-text/hud-overlay flags are off.
