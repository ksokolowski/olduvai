...
#include "presentation/game_render.hpp"    // FrameBuffer, blit_sprite
#include "presentation/image_out.hpp"      // capture_renderer_output
#include "presentation/smooth_present.hpp" // smooth_try_enable_vsync

namespace olduvai::presentation {
namespace {

// Whole-file byte reader (the codebase's recurring `slurp`; the shared-reader
...
void show_game_over_screen(const std::filesystem::path& game_dir,
    ...
    formats::CurArchive eva(slurp(game_dir / "FILESA.VGA"));
    formats::CurArchive efa(slurp(game_dir / "FILESA.CUR"));
    ...
    if (hd_scale > 1) {
        const auto up =
            enhance::upscale_rgba(end_fb.px, 320, 200, hd_scale, hd_profile);
        SDL_UpdateTexture(gtex, nullptr, up.data(), 320 * hd_scale * 4);
    } else {
...
void show_win_ending(const std::filesystem::path& game_dir, SdlAudio& audio,
    ...
    [&]() {
        ...
        auto render_at = [&](int render_y, Uint32 delay_ms) -> bool {
            ...
            if (hd_scale > 1) {
                const auto up = enhance::upscale_rgba(fb2.px, 320, 200,
                                                      hd_scale, hd_profile);
