...
// Option A: compose the center at NATIVE 320x200, assemble the wide native
...
// native 320x200 FrameBuffer via the read-only compose_static callback
...
// Composes its OWN native 320x200 center BACKGROUND (bg + tiles only, no
...
// Present a ready-made WIDE native buffer (native_w() x 200, NO baked HUD)
...
void WidescreenPresenter::present_transition(std::vector<std::uint8_t>& wide,
    ...
    // pre_upscaled: `wide` is ALREADY the HD buffer (native_w()*hd_scale x
...
void WidescreenPresenter::reapply_seam_bands(std::vector<std::uint8_t>& wide,
    ...
    for (const auto& [blo, bhi] : bands) {
        ...
        // ...and if the centre we were handed ALREADY had entities on it, put
        // them back — the tile pass above just painted background over them.
        //
        // The layering law ("authored tiles go back on top") is right for the
        // pan strip and the descent presenter, whose centre is an entity-FREE
        // background.  wrap_wide_static's centre is a fully composed frame,
        // so for it the same pass is an erase: every entity inside the band
        // vanishes.  Measured on L1 S2, whose S1|S2 rail bridge is the very
        // seam this function was written for — the bands there are
        // centre_native[0..64] and [256..320], and the two static spikes at
        // x=40 and x=60 sat inside the left one.  They were present in
        // 468/468 steady frames and only 40/148 cave-fade frames, the lower
        // one clipped at the band edge: the "spikes cut from the left during
        // the cave in/out fade" playtest report.
        //
        // advance_state=false: this is a re-draw of the same tick, and
        // draw_entities mutates club_flag once per real frame.
...
void WidescreenPresenter::wrap_wide_static(const FrameBuffer& center,
    ...
    // draw_player=FALSE, deliberately.  The centre handed to us already has
    // the player on it, drawn by compose_frame with the OUTGOING state — on a
    // cave entry that is the DESCENT sprite (back turned) at the entrance,
    // which is what the fade should show.  Re-drawing him here paints a second,
    // STANDING player over that: at the live position it is the cave's spawn
    // point ("the player jumps to the left as the fade starts"), and even
    // pinned to the outgoing position it is a duplicate standing sprite on the
    // surface.  The player belongs to the INCOMING cave frame, at the cave
    // entry position — not to this one.  Only the seam-band ENTITIES need
    // restoring here.
