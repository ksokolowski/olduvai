...
// ── Per-level BACKDROP recipes ──────────────────────────────────────────────
// These ran inline in build_screen_tiles, whose 97 cognitive-complexity points
// (BACKLOG §3.12) were mostly this: three unrelated recipes and a shared
// placement loop in one function.  They are moved verbatim, keep the file's
// purity contract (read-only, deterministic, no RNG, write only `out`), and
// each is now named after the thing it draws.

// L3: the GROT3 trunk column on screens 10/11, pine silhouettes everywhere
// else.  Net effect of the EXE's draw -> black-fill -> pine order.
...
// L7 ceiling screens (16..18 author the full-width stalactite border at
// y=9): the band above the ceiling is SOLID ROCK, not open lavarock —
// with only the backdrop row the stalactites hang mid-air under lava in
// the transparent HUD band.  Split out of push_l7_backdrop, which was
// still 56 points after the first split (BACKLOG §3.12); moved verbatim.
...
// L7: the lavarock backdrop and, on the ceiling screens, the rock band above
// it; screens 10-12 (the cave-warp area) get their visible floor/ceiling
// strips instead.  The matching CONTINUOUS COLLISION floor is a side effect
// and stays at the bind_screen call site.
